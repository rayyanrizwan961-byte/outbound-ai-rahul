"""OutboundAI — Supabase database layer (complete)."""
import os
import uuid
...
SENSITIVE_KEYS = {
    ...
    "S3_SECRET_ACCESS_KEY","CALCOM_API_KEY","DEEPGRAM_API_KEY",
}
...
def init_db() -> None:
    ...
    if not url or not key:
        print("SUPABASE_URL or SUPABASE_SERVICE_KEY not set — set them in .env")
        return
    try:
        db = _sdb()
        ...
        print("Run supabase_schema.sql in your Supabase SQL Editor first!")
...
async def get_all_settings() -> dict:
    ...
    for row in (result.data or []):
        ...
        out[k] = {"value": "" if k in SENSITIVE_KEYS else v, "configured": bool(v)}
    return out

async def save_settings(data: dict) -> None:
    db = await _adb()
    rows = [{"key": k, "value": str(v), "updated_at": datetime.now().isoformat()}
...
async def get_setting(key: str, default: str = "") -> str:
    ...
    if result and result.data: return result.data["value"]
...
async def insert_appointment(name: str, phone: str, date: str, time: str, service: str) -> str:
    ...
    await db.table("appointments").insert({
        ...
        "service": service, "status": "booked", "created_at": datetime.now().isoformat(),
    }).execute()
    return booking_id

async def check_slot(date: str, time: str) -> bool:
    db = await _adb()
    result = await (db.table("appointments").select("id")
...
async def get_next_available(date: str, time: str) -> str:
    ...
    return "no open slots in 7 days"
...
async def cancel_appointment(appointment_id: str) -> bool:
    db = await _adb()
    result = await (db.table("appointments").update({"status": "cancelled"})
...
# ── Call logs ─────────────────────────────────────────────────────────────
async def log_call(phone_number: str, lead_name: Optional[str], outcome: str, reason: str,
                   duration_seconds: int, recording_url: Optional[str]=None, notes: Optional[str]=None) -> None:
    db = await _adb()
    row = {
...
async def get_contacts() -> list:
    db = await _adb()
    rows = (await db.table("call_logs").select("*").order("timestamp", desc=True).execute()).data or []
    contacts: dict = {}
    for row in rows:
        phone = row["phone_number"]
        if phone not in contacts:
            contacts[phone] = {"phone_number": phone, "lead_name": row.get("lead_name"),
                               "total_calls": 0, "booked": 0,
                               "last_call": row["timestamp"], "last_outcome": row.get("outcome")}
        contacts[phone]["total_calls"] += 1
        if row.get("outcome") == "booked": contacts[phone]["booked"] += 1
...
# ── Campaigns ─────────────────────────────────────────────────────────────
async def create_campaign(name: str, contacts_json: str, schedule_type: str="once",
                          schedule_time: str="09:00", call_delay_seconds: int=3,
                          system_prompt: Optional[str]=None, agent_profile_id: Optional[str]=None) -> str:
    campaign_id = str(uuid.uuid4())
    db = await _adb()
    row = {"id": campaign_id, "name": name, "status": "active",
...
async def get_contact_memory(phone: str) -> list:
    db = await _adb()
    result = await (db.table("contact_memory").select("insight, created_at")
...
async def create_agent_profile(name: str, voice: str="Fenrir", model: str="gemini-3.1-flash-live-preview",
                                system_prompt: Optional[str]=None, enabled_tools: str="[]", is_default: bool=False) -> str:
